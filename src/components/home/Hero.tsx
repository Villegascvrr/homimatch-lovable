...
const Hero = () => {
  ...
  // Si el usuario está logueado, mostrar botón para ir a mantenimiento
  if (user) {
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            ...
            <p className="text-base md:text-lg text-muted-foreground mb-6 max-w-2xl mx-auto px-2">
              Gracias por registrarte. La aplicación estará disponible pronto.
            </p>

            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4 mb-8">
              <Button size={isMobile ? "default" : "lg"} className="rounded-full bg-gradient-to-r from-homi-purple to-homi-lightPurple hover:from-homi-lightPurple hover:to-homi-purple text-white font-bold shadow-lg shadow-purple-500/30 transform hover:scale-105 transition-all duration-300 w-full sm:w-auto" onClick={() => navigate('/maintenance')}>
                Ver estado de la app
              </Button>
              <Button size={isMobile ? "default" : "lg"} variant="outline" className="rounded-full w-full sm:w-auto" onClick={() => navigate('/matching')}>
                Explorar perfiles
              </Button>
            </div>
  ...
  // Si no está logueado, mostrar hero normal
  return <section className="relative overflow-hidden w-full lg:py-[30px] py-[5px]">
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-0 left-0 w-64 md:w-96 h-64 md:h-96 bg-homi-ultraLightPurple rounded-full opacity-50 blur-3xl -translate-x-1/2 -translate-y-1/2"></div>
        <div className="absolute bottom-0 right-0 w-64 md:w-96 h-64 md:h-96 bg-homi-ultraLightPurple rounded-full opacity-50 blur-3xl translate-x-1/3 translate-y-1/3"></div>
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="max-w-4xl mx-auto text-center space-y-8">
          <div className="inline-block px-4 py-1.5 mb-1 md:mb-2 rounded-full bg-homi-ultraLightPurple text-homi-purple text-xs md:text-sm font-medium animate-pulse-soft">
            ¡Ya disponible en todas las ciudades!
          </div>
          ...
          <p className="text-base md:text-xl text-muted-foreground mb-6 max-w-2xl mx-auto px-2">
            HomiMatch utiliza un sistema de matching para conectarte con compañeros de piso que comparten tus intereses y estilo de vida.
          </p>
