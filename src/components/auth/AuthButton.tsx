...
import { hasStoredSession, signInWithGoogleOAuth } from "@/lib/auth-helpers";
const GoogleIcon = () => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" className="mr-1">
...
const AuthButton = () => {
  ...
  if (!user) {
    ...
    if (isMobile) {
      return <div className="flex flex-col space-y-2" key={componentKey}>
          <Link to="/signin" className="block w-full text-center px-3 py-2 rounded-full text-base font-medium text-foreground hover:bg-muted">
            Iniciar sesión
          </Link>
          <Link to="/register" className="block w-full text-center px-3 py-2 rounded-full text-base font-medium bg-homi-purple text-white hover:bg-homi-purple/90">
            Registrarse
          </Link>
          <Button variant="outline" className="w-full flex items-center justify-center gap-2 rounded-full" onClick={handleGoogleSignIn} disabled={isGoogleSignIn}>
            {isGoogleSignIn ? <>
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-b-transparent"></div>
    ...
    return <div className="flex items-center gap-2" key={componentKey}>
        ...
        <Button variant="outline" size="sm" className="rounded-full flex items-center justify-center gap-1" onClick={handleGoogleSignIn} disabled={isGoogleSignIn}>
          {isGoogleSignIn ? <>
  ...
  return <DropdownMenu key={componentKey}>
